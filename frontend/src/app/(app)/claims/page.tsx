...
export default function ClaimsPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await fetchStatus();
      toast(`${data.summary?.total_claims?.toLocaleString() || ""} claims uploaded successfully`, "success");
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Upload failed. Please check the CSV format.";
      setError(msg);
      toast(msg, "error");
    } finally {
      setUploading(false);
  ...
  const handleReset = async () => {
    ...
    setSummary(null);
    try {
      const res = await fetch("/api/claims/reset", {
        method: "DELETE",
      });
      ...
      <div className="bg-white rounded-xl border border-gray-200/60 shadow-[var(--shadow-card)] p-6 mb-6">
        ...
        <div className="mt-4 flex items-center gap-3 justify-center">
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {uploading ? (
              <>
